...
export class ChatsComponent implements OnInit {
  ...
  setResult(ev: any) {
    if (ev.detail?.data?.action == 'gallery') {
      this.picImgToGallery();
    }

  }

  async picImgToGallery() {
    ...
    result.files.forEach((ele: any) => {
      console.log("ele.file",ele.file);
      console.log("ele.file.name",ele.file.name);
      console.log("type",ele.file.mimeType);
      // const rawFile = new File(file.blob, file.name, {
      //   type: file.mimeType,
      // });
       formData.append('file[]', ele.file, ele.file.name);
    })
